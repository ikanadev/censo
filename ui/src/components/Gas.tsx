import { type Cocina, numberAbbr, numberFormat } from "@/domain";
import { axisBottom, axisLeft, scaleBand, scaleLinear, select } from "d3";
import { For, onMount } from "solid-js";
import StatContainer from "./StatContainer";

const margin = { top: 24, right: 0, bottom: 100, left: 60 };
const size = {
	width: 1000,
...
export default function Gas(props: Props) {
	let gx: SVGGElement | undefined;
	let gy: SVGGElement | undefined;

	const max = Object.values(props.gas).reduce((max, d) => Math.max(max, d), 0);
	const total = Object.values(props.gas).reduce((sum, d) => sum + d, 0);

	const x = scaleBand()
		.domain(Object.keys(props.gas))
	...
	return (
		<StatContainer class="col-span-1 md:col-span-2" title="Combustible para cocina">
			<p>{numberFormat(total)} respuestas.</p>
			<svg viewBox={`0 0 ${size.width} ${size.height}`} class="w-full">
				<title>Combustible para cocina</title>
				...
				<For each={Object.entries(props.gas)}>{([key, qtty]) => (
					<>
						<text
